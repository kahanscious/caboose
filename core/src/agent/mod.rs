...
pub struct AgentLoop {
    ...
    pub cold_store: Option<ColdStore>,
    ...
    pub stashed_tool_defs: Vec<ToolDefinition>,
...
impl AgentLoop {
    ...
    pub fn start_stream(&mut self, provider: &dyn Provider, tool_defs: &[ToolDefinition]) {
    ...
    pub fn to_provider_messages(&self) -> Vec<provider::Message> {
        let mut out = Vec::new();
...
/// Normalize a tool path argument to be absolute, resolving against `primary_root`.
pub fn resolve_tool_path(primary_root: &Path, path: &str) -> String {
...
/// Normalize path fields in tool arguments.
pub fn normalize_path_field(
...
/// Normalize tool arguments based on tool name — resolve relative paths to absolute.
