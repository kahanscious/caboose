...
fn render_path_phase(f: &mut Frame, area: Rect, state: &WorkspaceAddState, colors: &Colors) {
    ...
    f.render_widget(
        Paragraph::new("path").style(Style::default().fg(colors.text_dim)),
        chunks[0],
    );
    f.render_widget(
...
fn render_name_phase(f: &mut Frame, area: Rect, state: &WorkspaceAddState, colors: &Colors) {
    let chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
            Constraint::Length(1), // label
    ...
    f.render_widget(
        Paragraph::new("name").style(Style::default().fg(colors.text_dim)),
        chunks[0],
    );
    f.render_widget(
...
fn render_mode_phase(f: &mut Frame, area: Rect, state: &WorkspaceAddState, colors: &Colors) {
    let options = [
    ...
    f.render_widget(
        Paragraph::new("search mode").style(Style::default().fg(colors.text_dim)),
        chunks[0],
    );
